...
const inventoryCheckCircuit = new CircuitBreaker(async (productId, quantity) => {
  const response = await axios.get(`${INVENTORY_SERVICE_URL}/${productId}/check?quantity=${quantity}`);
  return response.data;
}, circuitBreakerOptions);

// Create circuit breaker for inventory reservation
const inventoryReserveCircuit = new CircuitBreaker(async (productId, quantity) => {
  const response = await axios.post(`${INVENTORY_SERVICE_URL}/${productId}/reserve`, { quantity });
  return response.data;
}, circuitBreakerOptions);
...
[inventoryCheckCircuit, inventoryReserveCircuit, flakyTestCircuit].forEach(circuit => {
  circuit.on('open', () => console.log(`Circuit ${circuit.name} is open (not allowing requests)`));
  circuit.on('close', () => console.log(`Circuit ${circuit.name} is closed (allowing requests)`));
  circuit.on('halfOpen', () => console.log(`Circuit ${circuit.name} is half-open (testing if service is back)`));
  circuit.on('fallback', (result) => console.log(`Circuit ${circuit.name} fallback executed`, result));
});
...
router.post('/', async (req, res) => {
  try {
    ...
    try {
      const inventoryCheck = await inventoryCheckCircuit.fire(product_id, quantity);
      ...
      if (inventoryCheckCircuit.status === 'open' || inventoryReserveCircuit.status === 'open') {
        ...
        return res.status(202).json({
          message: 'Order created with pending status. Inventory service is currently unavailable.',
